        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div style={styles.card}>
            ...
            {employeeMessages.length === 0 && (
              <p style={styles.empty}>No messages from employees yet.</p>
            )}
            {employeeMessages.map((m, i) => (
              <div key={i} style={{ ...styles.notifCard, borderColor: m.read ? '#2a2a2a' : '#2255aa' }}>
                <div style={styles.notifHeader}>
                  <span style={{ ...styles.notifTarget, color: m.read ? '#555' : '#60aaff' }}>
                    From: {m.from}
                  </span>
                ...
                {!m.read && (
                  <button onClick={() => markRead(i)} style={styles.markReadBtn}>
                    ✓ Mark as Read
                  </button>
                )}
              </div>
